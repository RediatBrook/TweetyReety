<div class="flex">
    <form method="POST" action="/tweets/{{$tweet->id}}/like">
        @csrf
        <button type="submit"
                class="flex items-center mt-4 {{$tweet->isLikedBy(current_user())?'text-blue-500':'text-gray-500'}}">
            ...
            <span type="submit" class="text-xs">{{$tweet->likes?:0}}</span>
        </button>
    </form>
    <form method="POST" action="/tweets/{{$tweet->id}}/like">
        @csrf
        @method('DELETE')
        <button type="submit"
                class="flex items-center mt-4 ml-4 {{$tweet->isDislikedBy(current_user())?'text-red-400':'text-gray-500'}}">
            <svg viewBox="0 0 20 20" class=" mr-1 w-5">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class="fill-current">
                        <path
